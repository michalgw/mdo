...
type
  ...
  { TMDODataSetSortEngine }

  TMDODataSetSortEngine = class(TRxDBGridSortEngine)
  public
...
procedure TMDODataSetSortEngine.SortList(ListField: string; ADataSet: TDataSet;
...
begin
    ...
    for i := 0 to (ADataSet as TMDODataSet).Params.Count - 1 do
    begin
      Par := TParam(OldParams.Add);
      Par.Name := (ADataSet as TMDODataSet).Params[i].Name;
      Par.Value := (ADataSet as TMDODataSet).Params[i].Value;
    end;
//    OldParams.Assign((ADataSet as TMDODataSet).Params);
    cmd:=(ADataSet as TMDODataSet).SelectSQL.Text;
    strlen:=length(cmd);
    ...
    with TRegExpr.Create do begin
      ...
      (ADataSet as TMDODataSet).SelectSQL.Text:=s;
      //(ADataSet as TMDODataSet).Params.Assign(OldParams);
      for i := 0 to OldParams.Count - 1 do
        (ADataSet as TMDODataSet).ParamByName(OldParams[i].Name).Value := OldParams[i].Value;
      OldParams.Free;
...
initialization
  RegisterRxDBGridSortEngine(TMDODataSetSortEngine, 'TMDODataSet');
end.
